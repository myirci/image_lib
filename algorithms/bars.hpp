...
namespace SmpImgLib
{
    ...
    void bars(Image<T>& image, colorT<T> first_color, T increment, int bar_width) {
        do_bars(image, first_color, increment, bar_width, typename color_traits<T, colorT>::color_space_category());
    }
    ...
    void do_bars(Image<T>& image, colorT<T> first_color, T increment, int bar_width, color_rgb_tag) {
        ...
        for (int i = 0; i < image.num_columns(); ++i) {
            if (i % bar_width == 0) {
                value_r += increment;
                value_g += increment;
                value_b += increment;
        ...
        for (int i = 1; i < image.num_rows(); ++i) {
