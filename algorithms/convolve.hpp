...
namespace SmpImgLib
{
    ...
    private:
        int convert(int i, int j) const
        { // i: column (horizontal) index, j: row (vertical) index
    ...
    void apply_linear_filter(Image<T>& in_img, const filter_kernel& kernel, Image<T>& out_img)
    {
        ...
        else if (kernel.rows() > in_img.get_height() || kernel.columns() > in_img.get_width()) 
        {
        ...
        out_img.clear();
        out_img.set_rows(in_img.get_height());
        out_img.set_columns(in_img.get_width());
        out_img.set_channels(in_img.get_num_channels());
        out_img.get_buffer_size();
        out_img.set_color_space(in_img.get_color_space());
        for (int i = 0; i < in_img.get_num_channels(); ++i)
        {
            out_img.insert_channel(Channel<T>(out_img.get_height(), out_img.get_width()));
        }

        int row_start = (kernel.rows() - 1) / 2;
        int row_end = in_img.get_height() - row_start;
        int col_start = (kernel.columns() - 1) / 2;
        int col_end = in_img.get_width() - col_start;

        double sum;
        for (int t = 0; t < in_img.get_num_channels(); ++t) 
        {
