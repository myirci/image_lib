...
struct image_data {
    ...
    // default constructor
    image_data() : buff(nullptr) { }
    // constructor:
    image_data(int w, int h, int cc, J_COLOR_SPACE cs) : width(w), height(h), color_components(cc), color_space(cs) {
        allocate_memory();
    }
    // copy constructor:
    image_data(const image_data& im_data) : width(im_data.width), height(im_data.height),
        color_components(im_data.color_components), color_space(im_data.color_space) {
        allocate_memory();
